...
1. **(Manual)** Create the data folder and copy JSONs:
   ```
   ...
   ```
2. **(Manual)** Add `.venv/` and `logs/` to `.gitignore` (create it if it doesn't exist in `AskTV/`)
...
6. **(Manual)** Run in a persistent terminal session — this is a multi-day job:
   ```
   tmux new -s transcribe
   source .venv/bin/activate
   python scripts/transcribe_1980.py
   ```
   Or backgrounded with:
   ```
   nohup python scripts/transcribe_1980.py > logs/transcription_run.log 2>&1 &
   ```
